...
1. **Python 环境**: 必须使用 `Python 3.10` 或更高版本。[推荐3.11.9]
...
为了简化环境配置过程，项目根目录下提供了一个 `run.bat` 批处理脚本。**推荐Windows用户直接双击此脚本启动程序**。

该脚本会自动执行以下步骤：

1.  **[自动] 检查 Python 环境**：
    - 验证您的系统中是否已安装 Python 并将其添加到了系统路径 (PATH)。
2.  **[自动] 检查 MATLAB 引擎**：
    - 检测 Python 环境中是否已安装 MATLAB 引擎。如果已安装，则跳过安装步骤。
3.  **[手动] 输入 MATLAB 路径**：
    - 如果未检测到 MATLAB 引擎，脚本会提示您 **手动输入 MATLAB 的安装根目录**。
    - **示例**: `C:\Program Files\MATLAB\R2023a`
    - 您可以在 MATLAB 安装文件夹中，通过右键单击地址栏并选择“复制地址”来获取此路径【注意：目录下不能有中文，且根目录下必须要有extern文件夹】，如下图所示：
      ![复制MATLAB路径](image/README/matlab_path_example.png)
    - 请确保路径输入正确，否则引擎将无法安装。
4.  **[自动] 安装 MATLAB 引擎**：
    - 根据您提供的路径，自动完成 MATLAB 引擎的安装。
5.  **[自动] 安装项目依赖**：
    - 自动读取 `requirements.txt` 文件，并使用 `pip` 安装所有必需的 Python 库。
6.  **[自动] 启动应用程序**：
    - 完成所有检查和安装后，自动启动 `run_gui.py`，打开主程序界面。

通过此脚本，您无需手动执行任何 `pip install` 命令，大大简化了首次运行的准备工作。
