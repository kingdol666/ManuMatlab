...
class VisualizationManager:
        ...
        try:
            ...
            for i, res in enumerate(self.gui.simulation_step_results):
                if 'output' in res and 'T' in res['output'] and 't' in res['output']:
                    ...
                    target_temp = self.gui.model_manager.simulation_models[i].T_GunWen if i < len(self.gui.model_manager.simulation_models) else None
                    target_temp -= 273.15
                    # 根据模型类型和滚动方向创建更详细的名称
                    if model_type == "升温":
    ...
    def export_contour_data_to_csv(self):
        """Exports temperature contour data to CSV files."""
        if not self.gui.simulation_step_results:
            QMessageBox.warning(self.gui, "警告", "没有可导出的步骤结果")
            return

        # Let user select export directory
        export_dir = QFileDialog.getExistingDirectory(self.gui, "选择导出目录")
        if not export_dir:
            return

        try:
            # Get combined contour data
            plot_data, all_t_min, all_t_max, boundary_times = get_combined_contour_data(self.gui.simulation_step_results)

            if not plot_data:
                QMessageBox.warning(self.gui, "警告", "没有有效的步骤结果可以导出")
                return

            # Export combined contour data
            combined_data_file = os.path.join(export_dir, "combined_contour_data.csv")
            with open(combined_data_file, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                # Write header
                writer.writerow(['时间 (s)', '薄膜厚度 (m)', '温度 (K)'])
                
                # Write data for each plot
                for data in plot_data:
                    for i in range(data['xxxx'].shape[0]):
                        for j in range(data['xxxx'].shape[1]):
                            time_val = data['xxxx'][i, j]
                            thickness_val = data['yyyy'][i, j]
                            temp_val = data['tttt'][i, j]
                            writer.writerow([time_val, thickness_val, temp_val])

            # Export contour data per step
            for step_idx, data in enumerate(plot_data):
                step_file = os.path.join(export_dir, f"step_{step_idx+1}_contour_data.csv")
            ...
            # Export boundary times
            boundary_file = os.path.join(export_dir, "boundary_times.csv")
            with open(boundary_file, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(['边界序号', '时间 (s)'])
                for idx, boundary_time in enumerate(boundary_times):
                    writer.writerow([idx+1, boundary_time])

            QMessageBox.information(self.gui, "成功", f"温度云图数据已成功导出到 {export_dir}")
        except Exception as e:
            QMessageBox.critical(self.gui, "错误", f"导出温度云图数据失败: {str(e)}")
    ...
    def show_temperature_curves(self, key=None):
        ...
        try:
            ...
            for i, boundary in enumerate(model_boundaries):
                target_temp = boundary.get('target_temp')
                if target_temp is not None:
                    start_time = boundary.get('real_time', boundary['time_step'])
                    if i + 1 < len(model_boundaries):
                        end_time = model_boundaries[i+1].get('real_time', model_boundaries[i+1]['time_step'])
                    else:
                        end_time = time_steps[-1]
                    ax1.hlines(y=target_temp, xmin=start_time, xmax=end_time, color='gray', linestyles='--', linewidth=1.2)
            
            # 使用 mplcursors 实现悬停显示
            cursor = mplcursors.cursor(ax1, hover=True)
            @cursor.connect("add")
            def on_add(sel):
                # Find the corresponding boundary
                time_point = sel.target[0]
                boundary_text = ""
                target_temp_text = ""
                for boundary in reversed(model_boundaries):
                    if time_point >= boundary.get('real_time', boundary['time_step']):
                        boundary_name = boundary['name']
                        if "降温初始辊" in boundary_name:
                            boundary_name = "降温"
                        boundary_text = f"\n{boundary_name}"
                        if boundary.get('target_temp') is not None:
                            target_temp_text = f"\nTarget Temp: {boundary['target_temp']:.2f}°C"
                        break
                sel.annotation.set_text(f'{sel.artist.get_label()}{boundary_text}\nTime: {sel.target[0]:.2f}s\nTemp: {sel.target[1]:.2f}°C{target_temp_text}')
