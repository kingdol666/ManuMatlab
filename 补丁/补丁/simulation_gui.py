...
try:
    ...
    # 设置备用字体
    plt.rcParams['font.fantasy'] = ['SimHei', 'Microsoft YaHei']
    plt.rcParams['font.monospace'] = ['SimHei', 'Microsoft YaHei']
except:
...
class SimulationGUI(QMainWindow):
    ...
    def export_results_to_csv(self):
        ...
        if not hasattr(self, 'simulation_step_results') or not self.simulation_step_results:
            QMessageBox.warning(self, "警告", "没有可导出的仿真结果。")
            return
        ...
        try:
            # 为每个步骤导出结果
            for step_idx, step_result in enumerate(self.simulation_step_results):
                step_dir = os.path.join(export_dir, f"step_{step_idx+1}")
                os.makedirs(step_dir, exist_ok=True)
                
                model_id = step_result.get('model_id', step_idx+1)
                model_type = step_result.get('model_type', '')
                roll_direction = step_result.get('roll_direction', '')
                output = step_result.get('output', {})
                
                # 导出步骤信息
                info_file = os.path.join(step_dir, f"step_{model_id}_info.csv")
                with open(info_file, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(['模型ID', '模型类型', '辊方向'])
                    writer.writerow([model_id, model_type, roll_direction])
                ...
                for var_name, var_value in output.items():
                    ...
                    file_name = os.path.join(step_dir, f"{var_name}.csv")
                    
                    # 转换为numpy数组以便统一处理
                    var_array = np.array(var_value)
                    
                    with open(file_name, 'w', newline='', encoding='utf-8') as f:
                        writer = csv.writer(f)
                        
                        # 处理不同维度的数据
                        if var_array.ndim == 0:  # 标量
                            writer.writerow([var_name, var_array.item()])
                        elif var_array.ndim == 1:  # 一维数组
                            writer.writerow([var_name] + [i for i in range(len(var_array))])
                            writer.writerow(['值'] + var_array.tolist())
                        elif var_array.ndim == 2:  # 二维数组
                            # 写入表头
                            header = [var_name] + [i for i in range(var_array.shape[1])]
                            ...
                            # 保存完整维度信息
                            dim_info_file = os.path.join(step_dir, f"{var_name}_dimensions.csv")
                            with open(dim_info_file, 'w', newline='', encoding='utf-8') as dim_f:
                                dim_writer = csv.writer(dim_f)
                                dim_writer.writerow(['维度', '大小'])
                                for dim_idx, dim_size in enumerate(var_array.shape):
                                    dim_writer.writerow([dim_idx, dim_size])
                            
                            # 为每个三维及以上的数据保存多个二维切片
                            if var_array.ndim == 3:
                                # 三维数据：保存为多个二维切片
                                for i in range(var_array.shape[0]):
                                    slice_file = os.path.join(step_dir, f"{var_name}_slice_{i+1}.csv")
                                    with open(slice_file, 'w', newline='', encoding='utf-8') as slice_f:
                                        slice_writer = csv.writer(slice_f)
                                        # 写入表头
                                        slice_writer.writerow([f"{var_name}_slice_{i+1}"] + [j for j in range(var_array.shape[2])])
                                        # 写入数据
                                        for j, row in enumerate(var_array[i]):
                                            slice_writer.writerow([j] + row.tolist())
                            else:
                                # 四维及以上数据：先转换为三维，再保存切片
                                # 合并前两个维度
                                reshaped_3d = var_array.reshape(-1, var_array.shape[-2], var_array.shape[-1])
                                for i in range(reshaped_3d.shape[0]):
                                    slice_file = os.path.join(step_dir, f"{var_name}_slice_{i+1}.csv")
                                    with open(slice_file, 'w', newline='', encoding='utf-8') as slice_f:
                                        slice_writer = csv.writer(slice_f)
                                        # 写入表头
                                        slice_writer.writerow([f"{var_name}_slice_{i+1}"] + [j for j in range(reshaped_3d.shape[2])])
                                        # 写入数据
                                        for j, row in enumerate(reshaped_3d[i]):
                                            slice_writer.writerow([j] + row.tolist())
                
                self.update_log(f"步骤 #{model_id} 结果已导出到 {step_dir}")
            
            QMessageBox.information(self, "成功", f"所有结果已成功导出到 {export_dir}")
            self.update_log(f"所有结果已成功导出到 {export_dir}")
        except Exception as e:
            QMessageBox.critical(self, "错误", f"导出结果失败: {str(e)}")
            self.update_log(f"导出结果失败: {str(e)}", "error")
        ...
        try:
            config_data = {
                ...
                "main_params": {
                    ...
                    "t": self.t_edit.value() if hasattr(self, 't_edit') else 0.0,
    ...
    def _apply_config_to_ui(self, config_data):
        ...
        # 检查t_edit是否存在，如果存在则设置值
        if hasattr(self, 't_edit'):
